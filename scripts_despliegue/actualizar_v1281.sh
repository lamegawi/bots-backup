#!/bin/bash
# Actualizador v12.8.1 — 💰 /reclamar SIN falsas alarmas (cobros casados por
...
RESULT_FILE="/tmp/combos_update_v1281_${TS}.log"
exec > >(tee -a "$RESULT_FILE") 2>&1

HASH="9869c271"
MD5_ESPERADO="fec4801f7576ee24793866c838990016"
INSTALL_DIR="/opt/polymarket"
ESTADO="$INSTALL_DIR/combos_estado.json"

echo "=== ACTUALIZADOR v12.8.1 (💰 /reclamar sin falsas alarmas) - $(date) ==="
echo "HASH: $HASH · md5 esperado: $MD5_ESPERADO"

echo ""
echo "=== Paso 0: Copia de seguridad del estado ANTES de tocar nada ==="
if [ -f "$ESTADO" ]; then
  cp -a "$ESTADO" "$INSTALL_DIR/combos_estado.pre_v1281_${TS}.json"
  echo "OK: $INSTALL_DIR/combos_estado.pre_v1281_${TS}.json ($(wc -c < "$ESTADO") bytes)"
  python3 - "$ESTADO" <<'PY'
import json,sys
...
echo ""
echo "=== Paso 1: Descargar v12.8.1 a un .new y VALIDAR (el bot sigue corriendo) ==="
mkdir -p "$INSTALL_DIR"
...
[ -f "$INSTALL_DIR/poly_combos_bot.py" ] && cp -a "$INSTALL_DIR/poly_combos_bot.py" "$INSTALL_DIR/poly_combos_bot.pre_v1281_${TS}.py" && echo "backup del binario: poly_combos_bot.pre_v1281_${TS}.py"
...
echo ""
echo "=== Paso 3: Verificar funciones v12.8.1 (+ v12.8 / v12.7 / v12.6) ==="
grep -m 1 "POLY COMBOS BOT" "$INSTALL_DIR/poly_combos_bot.py" | head -1
for fn in "def saldo_token_op" "def _norm_titulo" "def _op_ts" "CTF_ERC1155" \
...
echo ""
echo "=== Paso 3b: Migracion - limpiar las marcas del aviso FALSO de v12.8 ==="
python3 - "$ESTADO" <<'PYEOF'
import json, sys, os
p = sys.argv[1]
if not os.path.exists(p):
    print("   sin estado, nada que migrar"); raise SystemExit
d = json.load(open(p))
av = d.get("reclamos_avisados") or {}
# v12.8 marcaba {"ts","auto","importe"}; v12.8.1 anade "titulo" o "nota"
basura = [k for k, v in av.items() if isinstance(v, dict) and v.get("auto")
          and not v.get("titulo") and not v.get("nota")]
if basura:
    for k in basura:
        print(f"   fuera marca falsa: {k[:22]}... importe={av[k].get('importe')}")
        av.pop(k, None)
    d["reclamos_avisados"] = av
    with open(p, "w") as f:
        json.dump(d, f, indent=2, ensure_ascii=False)
    print(f"   OK: {len(basura)} marca(s) eliminada(s); se reevaluaran con saldo on-chain")
else:
    print(f"   nada que limpiar ({len(av)} marca(s) en reclamos_avisados)")
PYEOF
...
echo ""
echo "=== EN TELEGRAM ==="
echo "  /reclamar        → ganadas SIN cobrar (ahora confirmado on-chain, sin falsas alarmas)"
...
if [ -n "$PAT" ]; then
  RUTA="diag_hetzner/combos_update_v1281_${TS}.log"
