#!/bin/bash
# Publica el log completo del bot de Combos a diag-public
set -e
TS=$(date +%Y%m%d_%H%M%S)
LOG="/tmp/full_log_${TS}.log"
exec > >(tee -a "$LOG") 2>&1
...
publicar() {
  local ruta="diag_hetzner/full_log_${TS}.log"
  [ -z "$PAT" ] && return
  ...
  local payload
  if [ -n "$sha" ]; then
    payload=$(python3 -c "import json; print(json.dumps({'message':'log ${TS}','content':'$b64','branch':'diag-public','sha':'$sha'}))")
  else
    payload=$(python3 -c "import json; print(json.dumps({'message':'log ${TS}','content':'$b64','branch':'diag-public'}))")
  fi
...
echo ""
echo "== LOG COMPLETO (ultimas 100 lineas) =="
tail -100 /var/log/poly-combos-bot.log 2>&1
...
echo ""
echo "== PROCESO python3 =="
ps aux | grep -E "poly_combos_bot" | grep -v grep | head -3

echo ""
echo "== ESPERAR 30s para ver siguiente pasada =="
sleep 30
echo "Despues de 30s:"
