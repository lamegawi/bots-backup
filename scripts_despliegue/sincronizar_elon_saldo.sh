#!/usr/bin/env bash
# sincronizar_elon_saldo.sh — sincroniza saldo del bot de Elon al real ($228.45)
set -u
TS=$(date -u +%Y%m%d_%H%M%S)
LOG=/tmp/sincronizar_elon_${TS}.log
...
new_pnl = new_saldo - new_saldo  # reset, ahora bankroll = saldo real
d['saldo'] = new_saldo
...
"
echo
echo "== 6. NO reiniciar el servicio (sigue corriendo) =="
...
echo "  ✅ historial: preservado (7 ops)"
echo "  ✅ backup: $ELON_DIR/real.json.bak_${TS}"
echo
echo "  ⚠️  El bot de Elon leerá el nuevo saldo en la próxima pasada (5 min)"
echo "  - NO se reinicia el servicio para no perder estado"
echo "  - Verifica logs: tail -f /opt/polymarket/bot-polymarket-elon/bot.log"
} > "$LOG" 2>&1
