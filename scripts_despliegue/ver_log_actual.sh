#!/bin/bash
# Publica el log ACTUAL del bot (no el del update)
set -e
TS=$(date +%Y%m%d_%H%M%S)
LOG="/tmp/live_log_${TS}.log"
exec > >(tee -a "$LOG") 2>&1
...
publicar() {
  local ruta="diag_hetzner/live_log_${TS}.log"
  [ -z "$PAT" ] && return
  ...
  local payload
  if [ -n "$sha" ]; then
    payload=$(python3 -c "import json; print(json.dumps({'message':'live ${TS}','content':'$b64','branch':'diag-public','sha':'$sha'}))")
  else
    payload=$(python3 -c "import json; print(json.dumps({'message':'live ${TS}','content':'$b64','branch':'diag-public'}))")
  fi
...
echo ""
echo "== LOG COMPLETO (ultimas 60 lineas) =="
tail -60 /var/log/poly-combos-bot.log 2>&1
echo ""
