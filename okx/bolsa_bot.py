...
def urls_analisis(yahoo):
    """Enlaces externos, sin credenciales, para ampliar el análisis."""
    q = urllib.parse.quote(yahoo, safe=".")
    return {
        "tecnico": f"https://www.tradingview.com/symbols/{q}/technicals/",
        "fundamental": f"https://finance.yahoo.com/quote/{q}/analysis/",
    }


def get_history(yahoo, rango="1y", intervalo="1d"):
    """Obtiene OHLC recientes de Yahoo para el resumen técnico."""
    url = (f"https://query1.finance.yahoo.com/v8/finance/chart/{urllib.parse.quote(yahoo)}"
           f"?range={rango}&interval={intervalo}&events=div%2Csplits")
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
    with urllib.request.urlopen(req, timeout=20) as r:
        d = json.loads(r.read().decode())
    ...
    highs = q.get("high") or []
    lows = q.get("low") or []
    return [(float(c), float(h), float(l)) for c, h, l in zip(closes, highs, lows)
...
def _rsi(closes, n=14):
    ...
    ganancias = [max(x, 0) for x in cambios[-n:]]
    perdidas = [max(-x, 0) for x in cambios[-n:]]
    media_g = sum(ganancias) / n
    media_p = sum(perdidas) / n
    if media_p == 0:
        return 100.0
    return 100 - (100 / (1 + media_g / media_p))


def analisis_tecnico(ticker, q):
    """Devuelve un análisis orientativo; no es asesoramiento financiero."""
    try:
        velas = get_history(ticker["yahoo"])
        cierres = [x[0] for x in velas]
        precio = float(q["precio"])
        sma20, sma50, sma200 = (_sma(cierres, n) for n in (20, 50, 200))
        rsi = _rsi(cierres)
        atr = None
        if len(velas) >= 15:
            tr = []
            for i, (_, high, low) in enumerate(velas[-14:]):
                previo = velas[-15 + i][0]
                tr.append(max(high - low, abs(high - previo), abs(low - previo)))
            atr = sum(tr) / len(tr)
        ...
        tendencia = "alcista" if sma20 and sma50 and precio > sma20 > sma50 else (
            "bajista" if sma20 and sma50 and precio < sma20 < sma50 else "mixta")
        # Dos escenarios prudentes: pullback y ruptura, con stop basado en ATR.
        if atr and tendencia == "alcista":
            entrada_pull = max(soporte or precio - atr, precio - atr)
            entrada_rupt = (resistencia + 0.1 * atr) if resistencia else precio
            stop = entrada_pull - 1.5 * atr
            entradas = f"pullback ~{entrada_pull:.2f} o ruptura >{entrada_rupt:.2f}"
        elif atr and tendencia == "bajista":
            entrada_pull = min(resistencia or precio + atr, precio + atr)
            entrada_rupt = (soporte - 0.1 * atr) if soporte else precio
            stop = entrada_pull + 1.5 * atr
            entradas = f"rebote/rechazo ~{entrada_pull:.2f} o pérdida <{entrada_rupt:.2f}"
        else:
            stop = (precio - 1.5 * atr) if atr else None
            entradas = "esperar confirmación; no hay señal limpia"
        return {"tendencia": tendencia, "rsi": rsi, "sma20": sma20,
                "sma50": sma50, "sma200": sma200, "atr": atr,
                "soporte": soporte, "resistencia": resistencia,
                "entradas": entradas, "stop": stop}
    except Exception as e:
        return {"error": str(e)}
...
def cmd_analisis(simbolo):
    cfg = cargar_config()
    ticker = next((t for t in cfg.get("tickers", [])
                   if t.get("simbolo", "").upper() == simbolo.upper()), None)
    if ticker is None:
        for p in cargar_cartera():
            if p.get("simbolo", "").upper() == simbolo.upper():
                ticker = p
                break
    if not ticker:
        enviar(f"No encuentro *{simbolo}* en seguimiento ni en cartera.", teclado())
        return
    try:
        q = buscar_quote(ticker["simbolo"])
    except Exception:
        q = None
    if not q:
        enviar(f"⚠️ No hay cotización actual para *{simbolo}*.", teclado())
        return
    a = analisis_tecnico(ticker, q)
    u = urls_analisis(ticker["yahoo"])
    lineas = [f"🔎 *ANÁLISIS — {ticker['simbolo']} {ticker.get('nombre', '')}*", "",
              f"Precio: {fmt_precio(q)} · Día: {fmt_pct(q['pct'])}"]
    if a.get("error"):
        lineas.append("\nNo pude calcular los indicadores ahora; usa los enlaces externos.")
    else:
        lineas += [f"Tendencia: *{a['tendencia']}*",
                    f"RSI(14): {a['rsi']:.1f}" if a["rsi"] is not None else "RSI(14): —",
                    "Medias: " + " · ".join(f"SMA{n} {a[x]:.2f}" for n, x in
                        ((20, "sma20"), (50, "sma50"), (200, "sma200")) if a[x] is not None),
                    f"Soporte 60d: {a['soporte']:.2f}" if a["soporte"] else "Soporte 60d: —",
                    f"Resistencia 60d: {a['resistencia']:.2f}" if a["resistencia"] else "Resistencia 60d: —",
                    f"\nPosibles entradas: {a['entradas']}",
                    f"Stop orientativo: {a['stop']:.2f}" if a["stop"] else "Stop orientativo: calcular con más datos"]
    lineas += ["", "⚠️ Orientativo, no asesoramiento financiero.",
               f"📊 Técnico: {u['tecnico']}", f"💼 Fundamental: {u['fundamental']}"]
    enviar("\n".join(lineas), kb_inline([[{"text": "📊 Técnico", "url": u["tecnico"]},
                                           {"text": "💼 Fundamental", "url": u["fundamental"]}],
                                          [{"text": "🔔 Crear alerta", "callback_data": f"alert_start:{ticker['simbolo']}"}]]))
...
def texto_precios(cfg, titulo="📊 *PRECIOS EN VIVO*"):
    ...
    for d in datos:
        ...
        u = urls_analisis(t["yahoo"])
        lineas.append(f"    🔎 Análisis: {u['tecnico']} · {u['fundamental']}")
    return "\n".join(lineas)
...
def cmd_precios():
    cfg = cargar_config()
    botones = [[{"text": f"🔎 {t['simbolo']}", "callback_data": f"ana:{t['simbolo']}"}]
               for t in cfg.get("tickers", [])]
    enviar(texto_precios(cfg), kb_inline(botones) if botones else None)
...
def empezar_alerta(simbolo_prefijado=None):
    cfg = cargar_config()
    disponibles = list(cfg.get("tickers", []))
    conocidos = {x.get("simbolo") for x in disponibles}
    for p in cargar_cartera():
        if p.get("simbolo") not in conocidos:
            disponibles.append(p)
    if simbolo_prefijado:
        alert_sel(simbolo_prefijado)
        return
    if not disponibles:
        enviar("No tienes acciones en seguimiento ni en cartera.", teclado())
        return
    ESPERA["paso"] = "alert_sel"
    ESPERA["datos"] = {}
    botones = [[{"text": f"{t.get('simbolo')} — {t.get('nombre', '')[:24]}",
                  "callback_data": f"alert_sel:{t.get('simbolo')}"}] for t in disponibles]
    botones.append([{"text": "❌ Cancelar", "callback_data": "alert_sel:cancelar"}])
    enviar("🔔 *Nueva alerta*\n\nElige el ticker:", kb_inline(botones))


def alert_sel(simbolo):
    if simbolo == "cancelar":
        ESPERA["paso"] = None
        enviar("❌ Cancelado.", teclado())
        return
    ESPERA["datos"] = {"simbolo": simbolo}
    ESPERA["paso"] = "alert_dir"
    enviar(f"🔔 *{simbolo}*\n\n¿Avisar cuando el precio…?",
           kb_inline([[{"text": "📈 suba por encima de", "callback_data": "alert_dir:above"},
                       {"text": "📉 baje por debajo de", "callback_data": "alert_dir:below"}],
                      [{"text": "❌ Cancelar", "callback_data": "alert_dir:cancelar"}]]))


def alert_dir(direccion):
    if direccion == "cancelar":
        ESPERA["paso"] = None
        enviar("❌ Cancelado.", teclado())
        return
    ESPERA["datos"]["direccion"] = direccion
    ESPERA["paso"] = "alert_precio"
    verbo = "por encima de" if direccion == "above" else "por debajo de"
    enviar(f"Escribe el precio {verbo} que quieres vigilar (ej. `100` o `100,50`).")


def alert_precio(txt):
    try:
        precio = float(str(txt).replace(",", ".").replace("€", "").replace("$", "").strip())
    except Exception:
        enviar("Escribe solo un precio numérico, por ejemplo `100,50`.")
        return
    if precio <= 0:
    ...
    estado.setdefault("personalizadas", [])
    # Evita duplicar exactamente la misma alerta activa.
    for a in estado["personalizadas"]:
        if (a.get("activo", True) and a.get("simbolo") == d["simbolo"]
                and a.get("direccion") == d["direccion"]
                and float(a.get("precio", 0)) == precio):
            enviar("ℹ️ Esa alerta ya existe.", teclado())
            ESPERA["paso"] = None
            return
    estado["personalizadas"].append({"id": int(time.time() * 1000),
        "simbolo": d["simbolo"], "direccion": d["direccion"],
        "precio": precio, "activo": True, "creada": datetime.now(MAD).isoformat()})
    guardar_alertas(estado)
    verbo = "suba por encima de" if d["direccion"] == "above" else "baje por debajo de"
    enviar(f"✅ Alerta creada: *{d['simbolo']}* cuando {verbo} *{precio:g}*.", teclado())
    ESPERA["paso"] = None
    ESPERA["datos"] = {}
...
def cmd_alertas():
    cfg = cargar_config()
    estado = cargar_alertas()
    personales = _alertas_personales(estado)
    lineas = [f"🔔 *ALERTAS (umbral diario {cfg['umbral_pct']:g}%)*", ""]
    activas = [a for a in personales if a.get("activo", True)]
    if activas:
        lineas.append("*Alertas de precio:* ")
        for a in activas:
            signo = "↑" if a.get("direccion") == "above" else "↓"
            lineas.append(f"• {a.get('simbolo')} {signo} {float(a.get('precio')):g}")
        lineas.append("")
    else:
        lineas.append("No hay alertas de precio personalizadas.\n")
    lineas.append("La alerta diaria avisa si un valor supera el umbral porcentual.")
    enviar("\n".join(lineas), kb_inline([[{"text": "➕ Añadir alerta", "callback_data": "alert_start"}]]))
...
def cmd_seguimiento():
    """Botón 👀 Seguimiento: muestra las acciones en la watchlist."""
    cfg = cargar_config()
    tickers = cfg["tickers"]
    if not tickers:
        enviar("👀 *Seguimiento*\n\n"
    ...
    for t in tickers:
        ...
        yahoo = t.get("yahoo", "") or ""
        linea = f"• *{t['simbolo']}*"
        if nombre:
            linea += f" — {nombre}"
        if yahoo:
            linea += f" _(ticker: {yahoo})_"
        lineas.append(linea)
    enviar("\n".join(lineas))
...
def procesar_callback(cb):
    ...
    if data.startswith("ana:"):
        ...
        empezar_alerta()
    elif data.startswith("alert_start:"):
        empezar_alerta(partes[1])
    elif data.startswith("alert_sel:"):
        alert_sel(partes[1])
...
def check_alertas():
    ...
    for d in datos:
        if not d["ok"]:
            continue
        t, q = d["ticker"], d["q"]
        if q["pct"] is None or abs(q["pct"]) < umbral or dia.get(t["simbolo"]):
            continue
        dia[t["simbolo"]] = True
        avisos.append((t, q))
    for a in estado.get("personalizadas", []):
        if not a.get("activo", True):
            continue
        t = next((x for x in cfg.get("tickers", []) if x.get("simbolo") == a.get("simbolo")), None)
        if not t:
            t = next((x for x in cargar_cartera() if x.get("simbolo") == a.get("simbolo")), None)
        if not t:
            continue
        try:
            q = buscar_quote(t["simbolo"])
            precio = float(q["precio"]) if q else None
            limite = float(a["precio"])
            cumple = precio is not None and ((a.get("direccion") == "above" and precio >= limite)
                                               or (a.get("direccion") == "below" and precio <= limite))
            if cumple:
                verbo = "por encima de" if a.get("direccion") == "above" else "por debajo de"
                enviar(f"🔔 *ALERTA DE PRECIO*: {t['simbolo']} está {verbo} {limite:g}.\n"
                       f"Precio actual: {fmt_precio(q)}.")
                a["activo"] = False
                a["disparada"] = datetime.now(MAD).isoformat()
        except Exception as e:
            print("alerta personalizada:", e)
    if avisos:
        for t, q in avisos:
            dir_ = "sube" if q["pct"] > 0 else "baja"
            enviar(f"🔔 *ALERTA*: {t['simbolo']} ({t['nombre']}) {dir_} "
                   f"*{q['pct']:+.2f}%* hoy.\nPrecio: {fmt_precio(q)}.")
    if avisos or estado.get("personalizadas"):
        estado[hoy] = dia
        guardar_alertas(estado)
