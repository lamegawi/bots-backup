...
def _descargar_jina(url):
    ...
    for intento in range(4):
        try:
            ...
            #   [@elonmusk](https://x.com/elonmusk)  [17h](https://twitter.com/elonmusk/status/ID)
            pat = re.compile(
                r"\[@elonmusk\]\(https://x\.com/elonmusk\)\s+\[([^\]]+)\]"
                r"\(https://(?:twitter\.com|x\.com)/elonmusk/status/(\d+)\)")
            ...
            for i, m in enumerate(posiciones):
                ...
                es_repost = bool(re.search(
                    r"\]\(https://(?:twitter\.com|x\.com)/(?!elonmusk/status/)"
...
def main():
    ...
    def una_pasada():
        ...
        if args.fuente in ("jina", "xcancel"):
            ...
            for nombre, fn in (("jina-tw", descargar_jina_tw),
