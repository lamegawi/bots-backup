...
# -*- coding: utf-8 -*-
"""
RECOGIDA AUTOMÁTICA DE TWEETS DE @realDonaldTrump — para mercados Polymarket
=====================================================================
Recoge la actividad de @realDonaldTrump (posts + quote posts + reposts;
SIN replies, igual que las reglas de resolución de los mercados
«Zelenskyy # tweets») y genera datos_trump.csv con el formato que
...
CSV = "datos_trump.csv"
ESTADO = "estado_tweets_trump.json"
RAW_DIR = "datos_raw_trump"
JINA_X = f"https://r.jina.ai/https://truthsocial.com/@{SCREEN}"
JINA_TW = f"https://r.jina.ai/https://truthsocial.com/@{SCREEN}"
    ...
    for intento in range(4):
        try:
            ...
            if ("Just a moment" in md or "Verifying your browser" in md
                    or "Rate limit" in md or "[@realDonaldTrump]" not in md):
            ...
            #   [@realDonaldTrump](https://x.com/realDonaldTrump)  [17h](https://twitter.com/realDonaldTrump/status/ID)
            pat = re.compile(
                r"\[@realDonaldTrump\]\(https://x\.com/realDonaldTrump\)\s+\[([^\]]+)\]"
                r"\(https://(?:twitter\.com|x\.com)/realDonaldTrump/status/(\d+)\)")
            ...
            for i, m in enumerate(posiciones):
                ...
                es_repost = bool(re.search(
                    r"\]\(https://(?:twitter\.com|x\.com)/(?!realDonaldTrump/status/)"
...
def main():
    ap = argparse.ArgumentParser(description="Recogida automática de tweets de @realDonaldTrump")
