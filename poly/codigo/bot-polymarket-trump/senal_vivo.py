...
Flujo completo y automático (ventana MENSUAL):
  ...
  2) calcula AVG7 / V2 / R / λ desde datos_trump.csv (o usa overrides)
...
def conteo_ventana(inicio_utc, ahora_utc):
    ...
    n = 0
    # ---- días completos (datos_trump.csv): d_inicio < d < hoy, o d == d_inicio
    # ---- si la ventana empieza a las 00:00 (mes natural)
    try:
        import csv as _csv
        with open("datos_trump.csv", newline="", encoding="utf-8") as f:
    ...
    for mk in mercados:
        ...
        for b in mk["bins"]:
            ...
            if lado:
                ...
                if p_lado < 0.10:
                    print(f"  · [FILTRO] descartado {b.get('titulo', '?')} {lado} "
...
def main():
    ap = argparse.ArgumentParser(description="Señal en vivo Polymarket · @realDonaldTrump (mensual)")
    ap.add_argument("--csv", default="datos_trump.csv", help="CSV de tweets (por defecto datos_trump.csv)")
