...
# -*- coding: utf-8 -*-
"""
MERCADOS POLYMARKET — «Trump # posts» (precios y cuotas en vivo)
=====================================================================
Descarga TODOS los mercados de Polymarket sobre el nº de tweets de
@realDonaldTrump (48h, semanales y mensuales), parsea los bins (rangos) y
...
GAMMA = "https://gamma-api.polymarket.com/public-search?q=%22trump%22+%22truth+social%22&limit=100"
...
def fetch_events():
    ...
    d = curl(GAMMA)
    eventos = [e for e in d.get("events", []) if ("posts" in (e.get("title") or "").lower() or "posts" in (e.get("title") or "").lower())]
    ...
    for i in range(-2, 16):            # 48h: desde hace 2 días hasta +15 (día a día)
        ...
        slugs.add(f"donald-trump-of-truth-social-posts-{MESES_NOMBRE[d1.month]}-{d1.day}-{MESES_NOMBRE[d2.month]}-{d2.day}-{d2.year}")
    ...
    for delta in (-2, -1, 0, 1, 2, 3):  # mensuales: 2 atrás, actual y 3 futuros
        ...
        slugs.add(f"donald-trump-of-truth-social-posts-{MESES_NOMBRE[d1.month]}-{d1.year}")
...
def parse_ventana(desc, end_iso, titulo):
    ...
    # ---- 48h / semanal: "from ... to ..."
    pat = re.compile(
        r"(?:from|between) ([A-Z][a-z]+) (\d{1,2})(?:,? (\d{4}))?[, \s]*(\d{1,2}):(\d{2}) ([AP]M) ET "
        r"(?:to|and) ([A-Z][a-z]+) (\d{1,2})(?:,? (\d{4}))?[, \s]*(\d{1,2}):(\d{2}) ([AP]M) ET")
...
def main():
    ap = argparse.ArgumentParser(description="Mercados Polymarket de Truth Social posts de @realDonaldTrump")
    ...
    if args.json:
        ...
        return
    print(f"Mercados Polymarket «Trump # posts»  (actualizado: "
