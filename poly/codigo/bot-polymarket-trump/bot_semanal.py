...
# -*- coding: utf-8 -*-
"""
🤖 BOT DEFINITIVO — Polymarket · Trump # tweets (paper trading)
====================================================================
Orquestador único que lo hace TODO en cada pasada:

  1) Recoge posts de @realDonaldTrump (jina/x.com con tiempos exactos +
     xcancel de respaldo), fusiona con el estado y actualiza
     datos_trump.csv (guard monótono: solo sube, nunca degrada).
...
LOG = "bot_trump.log"
EXCEL_HIST = "Historial_Operaciones_Trump.xlsx"
ESTADO_BOT = "estado_bot_trump.json"
...
# ------------------------------------------------------------------- 1) tweets
def recoger(opts):
    log("1/4 · Recogiendo posts de @realDonaldTrump…")
    vistos_n = {}
    ...
    if not vistos_n:
        ...
        try:
            notificar.alerta_error(
                "⚠️ No se pudieron recoger posts de @realDonaldTrump (todas las fuentes "
...
def pasada(opts):
    t0 = time.time()
    log("=" * 62)
    log("PASADA COMPLETA (TRUMP)" + (f"  ·  MODO: {opts.modo.upper()}" if opts.modo == "real" else ""))
    recoger(opts)
    actualizar_mercado()
    trading(opts)
    avisar_casi_senal()
...
def ver_estado():
    ...
    try:
        ...
        print(f"  papel: {e}")
    print("\nArchivos clave: datos_trump.csv · estado_tweets_trump.json · mercado_activo.json · "
          "papel.json · resultados_papel.csv · bot.log")


def main():
    ap = argparse.ArgumentParser(description="Bot definitivo — Polymarket · Trump # tweets")
