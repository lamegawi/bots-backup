...
# -*- coding: utf-8 -*-
"""
MERCADOS POLYMARKET — «Zelenskyy # posts» (precios y cuotas en vivo)
=====================================================================
...
GAMMA = "https://gamma-api.polymarket.com/public-search?q=%22zelenskyy%22&limit=100"
...
def fetch_events():
    ...
    d = curl(GAMMA)
    eventos = [e for e in d.get("events", []) if ("tweets" in (e.get("title") or "").lower() or "posts" in (e.get("title") or "").lower())]
    ...
    for i in range(-2, 16):            # 48h: desde hace 2 días hasta +15 (día a día)
        ...
        slugs.add(f"zelenskyy-of-tweets-{MESES_NOMBRE[d1.month]}-{d1.day}-{MESES_NOMBRE[d2.month]}-{d2.day}-{d2.year}")
    ...
    for delta in (-2, -1, 0, 1, 2, 3):  # mensuales: 2 atrás, actual y 3 futuros
        ...
        slugs.add(f"zelenskyy-of-tweets-{MESES_NOMBRE[d1.month]}-{d1.year}")
...
def main():
    ...
    if args.json:
        ...
        return
    print(f"Mercados Polymarket «Zelenskyy # posts»  (actualizado: "
