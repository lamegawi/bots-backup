    ...
    try:
        ...
        for m in mercados:
            if m.get("slug") == act.get("slug"):
                return _posts_en_ventana(m.get("inicio_iso"), m.get("fin_iso"))
    except Exception:
        pass
    return None


def _posts_en_ventana(inicio_iso, fin_iso):
...
def resumen_diario(saldo, paso, historial, apuesta_activa=None,
    ...
    """Aviso diario: estado + cada ventana registrada con posts y apuestas."""
    hoy = datetime.now(ET).date()
    ...
    if lista:
        ...
        for m in lista:
            ...
            tw = _posts_en_ventana(m.get("inicio_iso"), m.get("fin_iso"))
            linea = f"🪟 {nombre}"
            if tw is not None:
                linea += f" · {tw} posts"
...
if __name__ == "__main__":
    ...
    if args.test:
        res = enviar("🧪 Mensaje de prueba del bot de Polymarket. "
                     ...
                     titulo="Test bot Zelenskyy", etiqueta="robot")
        print("Canales:", res)
        cfg = cargar_config()
        nt = cfg.get("ntfy", {})
        if nt.get("topic"):
