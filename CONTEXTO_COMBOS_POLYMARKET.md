...
- 11 sept ~22:40 (Madrid) — **v12.8.2 DESPLEGADA Y VERIFICADA EN HETZNER + LAS 4 DE AGOSTO YA ESTÁN EN EL LIBRO** — log `combos_update_v1282_20260911_201116.log` (20:11 UTC): paso 0 copia del estado (117504 bytes, 9 abiertas · 41 archivadas · PnL −$21.72); paso 1 md5 `829d3b33…` idéntico + compila; paso 3 **35/35 marcadores OK**; **paso 3b `INYECTADAS 4 ops GANADAS de agosto: pagado $50.00 -> a cobrar $82.97 (+$32.97)`** con sus 4 tokens; paso 4 `active (running)` 20:11:20; paso 5 `Test proxy: 200` + **`v12.8.2 iniciado`** + auto-curación **confirmadas=45, 0 reabiertas, 0 corregidas, PnL −21.72 → +11.26** (estable: 11.26→11.26) + **`💰 reclamar: 4 ganada(s) sin cobrar (~$82.98) → aviso`** (éste es REAL y sólo se manda una vez); paso 6 `45 archivadas (✅27 ❌18)` y `reclamos avisados: 7 · $82.98` (3 marcas antiguas de saldo 0 + las 4 nuevas con 19.05/19.32/14.04/30.57); paso 6b confirma en vivo `pid→404` vs token real para las 2 combos abiertas (siguen n/d porque la leg de Seyboth Wild está cerrada **sin ganador declarado**, UMA pendiente, y la del Real Madrid ya la ganó *Over*). ⚠️ **OJO CON LA LECTURA DEL PANEL**: el PnL archivado de **+$11.26** incluye los **+$32.98 teóricos** de las 4 de agosto —ese dinero NO está en la cartera hasta que se reclame por la web o por soporte—. **VALIDADO el mensaje de `/reclamar` antes de que lo pida el user**: 1322 chars (un solo envío, `enviar_largo` corta a 3900), entidades Markdown balanceadas (28 `*`, 5 `_`, 10 backticks, 0 corchetes sueltos) ⇒ no se repetirá el `telegram_api error: HTTP Error 400` de las 18:10 (v12.8.1). **SIN enlace al evento**: `_enlace_evento()` usa Gamma `/markets?condition_ids=<cid de leg>` y para mercados de **agosto** Gamma devuelve **0 elementos** (sí funciona con los de septiembre) ⇒ el mensaje sale sin link; los enlaces siguen en `v12.7_cerradas_reales/4_COMBOS_SIN_COBRAR.md` y en `v12.8_reclamar/SOPORTE_POLYMARKET_82.97.md`. Posible mejora futura: guardar el `slug` del evento en las legs y usarlo como fallback. **Cosmética corregida en el repo** (commit de abajo, NO hace falta redesplegar): dos `echo` del actualizador llevaban `$82.97` y `$0` sin escapar y el shell se los comió (en el log salieron "(2.97)" y "(actualizar_v1282.sh)"). Ruido benigno en el log: `loop err: The read operation timed out` y el rechazo `stake_auto_0_sin_margen` de las 18:15 (RFQ 1.39 vs estimada 1.44 ⇒ el bot NO aceptó, $0 gastados) — ambos funcionan como deben.
- 11 sept ~22:20 (Madrid) — **PUSH DE v12.8.2 COMPLETADO (con la inyección de las 4 de agosto)** — el user dio un PAT nuevo; `fetch` de la rama compartida `arena/01a058fe-bots-backup` → el remoto seguía en `ecda4d2` (no había avanzado) ⇒ push fast-forward **`ecda4d2..ffc0435`** con los 3 commits: **`e3af3dfc`** (bot v12.8.2, md5 `829d3b335d993a205ef7453f0bec0eb8`, 4309 líneas) → **`c0ed5c06`** (`actualizar_v1282.sh` + paso 6b) → **`ffc0435`** (paso 3b de inyección + `ops_agosto_4.json` + tests 39/39 + este CONTEXTO). Token borrado del config en la misma llamada (0 coincidencias de `github_pat`); **avisado de revocarlo**. **VERIFICADO POR RAW ANTES de dar los comandos** (lección del incidente del 8-sep): bot en `e3af3dfc` → HTTP 200, md5 idéntico, `py_compile` OK, cabecera `POLY COMBOS BOT v12.8.2`; actualizador en `ffc0435` → HTTP 200, 22384 bytes/513 líneas, `bash -n` OK, `HASH="e3af3dfc"`, `MD5_ESPERADO="829d3b33…"`, pasos 0/1/2/3/**3b**/4/5/6/**6b** presentes, y el payload viaja embebido (4 tokens ×2, 4 tx de compra, 18 `0x…` únicos = 10 condition_id de legs + 4 conditionId sintéticos de parlay). Despliegue pendiente de ejecución por el user con el hash `ffc0435`; al arrancar se espera: inyección de las 4 ops, auditoría con 4 confirmadas, aviso 💰 real de 4 · $82.98 y `/reclamar` listándolas con token y enlace.
- 11 sept ~22:00 (Madrid) — **v12.8.2 + INYECCIÓN DE LAS 4 DE AGOSTO (aprobadas por el user) listas para push** — el user decidió (ask_user): **PAT nuevo** para el push, **SÍ inyectar** las 4 ganadas de agosto en el libro del bot, y **/testcerrar nada más desplegar**. **PASO 3b NUEVO en `actualizar_v1282.sh`**: inyecta en `historial` las 4 ops con sus datos REALES (payload `scripts_despliegue/ops_agosto_4.json`, embebido en el script): token del parlay, tx de compra, stake, shares, `combo_condition_id` sintético y —lo que faltaba— los **`condition_id` reales de las 10 legs** con la cara ganadora, recuperados de Gamma (`events?slug=`) y confirmados en el CLOB (`winner=true`). Es **idempotente** (compara por token), escribe con `tempfile`+`os.replace` (atómico) y el paso 0 ya hace copia del estado. **VERIFICADO EN VIVO con el bot**: `pendientes_reclamar(con_saldo=True)` → **4 reclamables · $82.98** (suma de saldos redondeados; el exacto es $82.973) con `fuente=legs_ganadas` y `sin_datos=0`; `reauditar_estado(seco)` → **4 confirmadas, 0 corregidas, 0 reabiertas, PnL +$32.98 → +$32.98** (queda estable, `AUDITORIA_DIAS_MAX=45` y agosto cae dentro a 39 días, pero como el PnL inyectado es `shares − stake` —la misma fórmula de `resolver_operacion`— no hay corrección); `resolver_operacion` devuelve ganada/pnl exacto (+4.05, +4.32, +4.04, +20.57) y `fin` real por leg; `preflight_redeem` responde **(False, "revert: sólo la cuenta de Polymarket 0xac9930b2…5294 puede ejecutar redeem()")** ⇒ el `/reclamar` del servidor enseñará ese motivo y no "sin datos". Al arrancar, `reclamar_check()` mandará UN aviso 💰 real de 4 · $82.98 con enlaces y lo marcará en `reclamos_avisados`. ⚠️ **LECCIÓN DE ESTA COMPROBACIÓN (casi me llevo por delante un dato bueno)**: al recuperar los `condition_id` por slug cogí `events[0].markets[0]`, que en esos eventos es el mercado **"Game 1 Winner"**, no el ganador del partido ⇒ el CLOB decía que 3 legs las había ganado el OTRO equipo (LGD, 3BL, Lupus) y parecía que los combos #1 y #3 eran perdedores y que mi localizador `4_COMBOS_SIN_COBRAR.md` estaba mal. Al casar por el **título exacto** del `/activity` (`title` con las legs unidas por " AND ") las 10 legs dan `winner=true` en la cara comprada (la primera) ⇒ **los $82.97 siguen siendo reales y sin cobrar**. Regla: nunca fiarse de `markets[0]` de un evento; casar por `question` exacto. **PASO 6b NUEVO**: tras reiniciar, el script imprime leg a leg de las abiertas `pid→404 · token real→200 · mid` y el precio vivo resultante (misma regla de veredicto que `estado_leg()`) para que el log demuestre que el camino de precios ya funciona; las 2 combos abiertas de hoy (7-8 sept) siguen sin precio porque una leg (`Seyboth Wild`) está cerrada **sin ganador declarado** (UMA pendiente) y la otra ya ganó (`Over`). Tests: **39/39** (`test_v1282.py`, con 4 checks del payload) + 59/59 + 28/28 + 28/28 = **154 en verde**. Commits locales: bot **`e3af3dfc`** · scripts **`c0ed5c06`** + el de la inyección (abajo). **PENDIENTE: el PAT para push y el despliegue.**
- 11 sept ~21:10 (Madrid) — **v12.8.1 DESPLEGADA Y VERIFICADA + v12.8.2 preparada (🔒 el cierre de un combo vuelve a ser posible)** — el user desplegó la v12.8.1 a las **18:05 UTC** (log `combos_update_v1281_20260911_180512.log`): md5 idéntico, 28/28 funciones, el paso 3b borró las **3 marcas falsas** del aviso de la v12.8 (`reclamos_avisados: 3 · $0.00` con nota "saldo 0 on-chain: ya cobrada/vendida"), servicio `active (running)`, `Test proxy: 200`, **`v12.8.1 iniciado`**, y la auto-curación re-evaluó con saldo on-chain → **0 sin cobrar** (fin de la falsa alarma; 2ª pasada idempotente: 0 cambios, 41 confirmadas, PnL −$21.72 → −$21.72). En ese mismo log aparecieron **dos cosas nuevas**: (1) a las **17:49:56** el user pulsó 🔒 dos veces y el bot respondió `[CIERRE] sin precio vivo: no se puede dimensionar la venta`; (2) a las **17:54** su `/reclamar` manual dijo `pre-flight redeem: sin datos · RPC respondió: Too Many Requests, Please apply an OnFinality API key` (era `1rpc.io` reenviando el 429 de su proveedor; el resultado `0 sin cobrar · revisadas 23` era correcto). **CAUSA RAÍZ DE (1), VERIFICADA EN VIVO**: `vivo_de()` pedía el mid con `legs[].position_id`, que es el id del **CATÁLOGO** de combos y NO el `token_id` del CLOB — el CLOB responde **404 SIEMPRE** a esos ids (comprobado con 2 mercados reales del catálogo: position_id → 404; token real → 200 con mid 0.155/0.845 y 0.0385/0.9615). Es **el mismo error de ids que en v12.6 rompía los resultados**, pero en la ruta de precios: ningún combo tenía precio vivo ⇒ 🔒 y 🧪 `/testcerrar` acababan siempre en "sin precio vivo" y el panel 📂 mostraba `📈 precio ahora: n/d` (y los botones 🔒 sin valor estimado). Con los datos reales del servidor (volcado `estado_combos_v127_…`) las 9 abiertas de hoy ya están **decididas**, por eso tampoco cotizaban; pero una leg del estado (`Genoa: Thiago Seyboth Wild…`, cid `0xe0b5ba4d…`, pid `724302561751…`, outcome null) resuelve ahora a su token real `92783053747398…` por índice 0. **FIX v12.8.2** (`parche_v1282.py`, 10 ediciones, md5 **`829d3b335d993a205ef7453f0bec0eb8`**, 4309 líneas): nuevo `token_clob_de_leg(lg)` (token_id REAL por condition_id + outcome, o índice 0 si la op es antigua; caché 15 min) y `vivo_de()` valorando cada leg con él; si una leg ya está **decidida** (su mid es 1.00/0.00 y `precio_mid` lo descarta) se cuenta por su veredicto vía `estado_leg()` ⇒ un combo con una leg terminada y el resto en juego **sí** tiene valor (antes "n/d"); `cerrar_grupo()` reordenado con **guardia de resuelta ANTES del precio** (con las legs decididas el producto da 1.00 y habría intentado VENDER algo que ya no cotiza) → mensajes "ya está COBRADA" / "se COBRA (/reclamar)" / "valor 0 (/reauditar)" / nuevo `sin_valor` cuando una leg perdió / `sin_precio_vivo` sólo si de verdad no cotiza; `/cerrar` traduce los motivos (antes enseñaba el código crudo); panel 📂 dice `n/d (posición resuelta: ya no cotiza)`; y anti-429: pausa de 0,15 s antes de cada `eth_call` real (sólo en fallo de caché, no ralentiza el AUTO) + reintento único tras 1,2 s saltando la caché (que guardaba el `None` 5 min) en `pendientes_reclamar`. **NO se toca el RFQ**: los `leg_position_ids` del catálogo son los correctos para la API de combos. Tests **35/35** (`test_v1282.py`, 6 en vivo contra el catálogo y el CLOB) + 59/59 + 28/28 + 28/28 = **150 pruebas en verde**. **PENDIENTE: push de v12.8.2 (hace falta un PAT nuevo — el anterior conviene revocarlo) y despliegue en Hetzner; después ya se puede hacer el `/testcerrar 1` que llevaba pendiente desde la v12.3.**
- 11 sept ~20:15 (Madrid) — **v12.8 DESPLEGADA en Hetzner (17:47 UTC, log `combos_update_v128_20260911_174722.log`) + v12.8.1 (fix de la falsa alarma)** — el user ejecutó primero la v12.7 (17:36 UTC, log `combos_update_v127_20260911_173643.log`): la auto-curación hizo su trabajo de verdad → **PnL archivadas −$276.68 → −$21.72**, 6 reabiertas, 16 corregidas, 25 confirmadas; y a las 17:47 la v12.8 arrancó limpia (md5 idéntico, 21/21 funciones, `Test proxy: 200`, `v12.8 iniciado`) con una 2ª auditoría **idempotente** (0 cambios, 41 confirmadas). ⚠️ **PERO** el 💰 automático avisó "7 ganadas sin cobrar (~$47.58)" y era **FALSO**: volcados de estado publicados en diag-public (`estado_combos_v127_20260911_173642.json` / `_174720.json`) + cadena mostraron que son 7 ops de **MERCADO SIMPLE** de la era copy-trading (`fuente_verdad=mercado_simple`, 7-8 sept, stake $5): Cagliari ×4, LoL IG-LGD, SMU-FSU ×2, con **saldo 0** en el CTF y en el ERC1155 de parlays, y sus cobros SÍ existen en data-api (Cagliari **$27.13** en UN redeem que agrupa las 4 compras, SMU **$12.58**, LoL **$7.87**). Tres causas: (a) `cobros_wallet()` pedía `/activity?limit=500` **sin filtro de tipo** ⇒ los trades empujaban fuera de la ventana los cobros antiguos (48 REDEEM visibles de **77**); (b) esas ops guardan un id de token que **no** es el `asset` del cobro ⇒ el índice por token nunca casaba; (c) `reclamar_check()` no deduplicaba por token (7 ops → 3 tokens, de ahí los "$47.58" inflados) ni confirmaba on-chain. **FIX v12.8.1** (`parche_v1281.py`): `/activity?type=REDEEM` con `COBROS_PAGINAS=2` (histórico completo desde abril) + índice por **título normalizado** (`_norm_titulo`: minúsculas, sin signos, " AND " ↔ " + ") con **ventana de 21 días** (`_op_ts`) para no casar un cobro con otra op del mismo título; `verdad_real()` anota en ese caso **sus shares** (1 token ganador = $1), nunca el total del redeem; `saldo_combo_token(contrato=…)` + `saldo_token_op()` (combos → parlay ERC1155, simples → CTF `0x4D97…6045`); `reclamar_check()` agrupa por token, usa el **saldo real** como importe y **sólo avisa con saldo > 0** (las que dan 0 se marcan con la nota "saldo 0 on-chain: ya cobrada/vendida" y no se vuelven a mirar); `pendientes_reclamar()` mira el contrato que toca. **VERIFICADO EN VIVO**: las 7 ops reales casan por título, saldo 0 en el CTF, y las 4 de agosto siguen sin cobrar (**$82.97**). Tests **28/28** (`test_v1281.py`) + 59/59 + 28/28 = **115 pruebas**. 🔎 **DATO CLAVE**: las 4 ganadas de agosto **NO están en el `combos_estado.json` del servidor** (0/4 — el estado sólo tiene 50 ops de septiembre), así que `/reclamar` no las ve: viven en `v12.7_cerradas_reales/4_COMBOS_SIN_COBRAR.md` y en el texto para soporte `v12.8_reclamar/SOPORTE_POLYMARKET_82.97.md`. Tampoco salen en data-api `/positions` (ese endpoint no incluye parlays: 42 posiciones, todas de mercados normales, y `redeemable=true` aparece incluso en posiciones sin valor). Commits: bot **`2187f7a1`** (v12.8) → **v12.8.1** (hash abajo) · scripts `actualizar_v128.sh` (`a9b5efa2`, descarga y valida ANTES de parar el bot) y `actualizar_v1281.sh` (+ migración que limpia las 3 marcas del aviso falso).
- 11 sept ~19:30 (Madrid) — **v12.8 (petición user: "mirar cómo ejecutar el redeem on-chain y añadir un `/reclamar` al bot en la v12.8")** — **PUSH DE v12.7 COMPLETADO** antes de nada: el user dio un PAT fine-grained, `fetch` de la rama compartida (el remoto había avanzado a `4a58584` con la sesión del bot de Elon) → `merge` limpio sin conflictos → **`5b77668`** → push `4a58584..5b77668` OK; verificado por raw: `4731f473/scripts_despliegue/poly_combos_bot.py` 200 con md5 `7af825202718d778a8099bb11b23add` idéntico. (Avisado: **revocar el PAT**.) **INVESTIGACIÓN DEL REDEEM DE PARLAYS** (para `/reclamar`): decodificada una tx de cobro real → es un UserOp **ERC-4337 v0.7** (`handleOps` en `0x0000000071727De22E5E9d8BAf0edAc6f37da032`, tx a `0x84ba8962…`) cuya cuenta `0xac9930b2ae455a671b62de86876a7e8587825294` hace `execute(bytes32,bytes)` → llamada interna al **adaptador de parlays `0xa1200000d0002264c9a1698e001292d00e1b00af`** con `redeem(address[] owners, uint256[] tokenIds)` (sel `0x53d190cf`, **arrays paralelos** owners[i]↔tokenIds[i], owners = la proxy wallet, tokenIds = el token del combo; el `conditionId` que da data-api para un parlay **ES** ese token id). Mi codificador ABI reproduce el calldata real **byte a byte**. **Simulaciones `eth_call`**: desde la proxy wallet, desde una EOA aleatoria y desde `0x1` → **REVERT**; desde la cuenta `0xac9930b2…` → **OK** (también un lote paralelo de 4 tokens; longitudes desiguales → revert) ⇒ el adaptador es **sólo-para-el-operador**: la wallet del user NO puede auto-redimir parlays. Barrido de selectores alternativos (manager `0x3000…31a8`, ERC1155 `0x006f…efef`, adaptador) y de endpoints de combos (positions/redeem/claims/health) → nada útil (revert/404). Fuentes del relayer oficial descargadas en `relayer_src/` (`py-builder-relayer-client`): el flujo PROXY firma un struct `rlx:` y lo ejecuta el relay_hub con `msg.sender` = la proxy ⇒ **dead-end** para parlays hoy. **IMPLEMENTACIÓN v12.8** (`parche_v128.py`, 15+2 ediciones): `calldata_redeem()`, `preflight_redeem()` (eth_call con caché 24 h y distinción revert/red caída vía el nuevo `_rpc_eth_call_ex`), `pendientes_reclamar()` (saldo on-chain + legs ganadoras + sin REDEEM, dedup por token, importe = saldo porque 1 token ganador = $1), `_enlace_evento()` (gamma `?condition_ids=` → `polymarket.com/event/<slug>`), `_texto_reclamar()`, `_marcar_avisados()`, `reclamar_check()` (enganchado a la auto-curación, aviso único) y `cmd_reclamar()` en hilo con `AUDITORIA_LOCK`+`PASADA_LOCK`; `/reclamar [prueba]` + botón 💰 Reclamar; `/status` ahora sí muestra 🩺 (la v12.7 calculaba `_aud_txt` y **no** lo metía en el mensaje) y 💰 sin cobrar; `RPCS_POLYGON` sin el RPC muerto. **TESTS 59/59 + regresión 28/28**; LIVE: los 4 saldos siguen en la wallet (**$82.97**) y el adaptador sigue rechazándonos. **PENDIENTE: push de v12.8 (hace falta PAT) + despliegue en Hetzner.**
- 11 sept 18:40 (Madrid) — **v12.7 (denuncia del user: "las ops de ✅ Cerradas NO son reales; cantidades y resultados no coinciden con Polymarket y hay combos cerrados antes de terminar")** — **CAUSA RAÍZ VERIFICADA**: el catálogo `/v1/rfq/combo-markets` devuelve `position_ids` que **NO son los `token_id` del CLOB** (otro espacio de ids: pares consecutivos …6064/…6065; p.ej. "U.S. invade Iran" catálogo `79855995…6064` vs CLOB `5511507842…`), así que `resolver_operacion` hacía `lg["position_id"] == win_tok` y **nunca** acertaba ⇒ en cuanto una leg cerraba, el combo se archivaba como **PERDIDA (pnl = −stake)** aunque ganara. Segundo bug: un mercado `closed` **sin ganador declarado** (resolución UMA pendiente) también contaba como pérdida ⇒ "cerrados antes de terminar". **DESCUBRIMIENTOS ON-CHAIN**: los tokens de combo viven en el ERC1155 `0x006f54f7f9a22e0000cc2ab60031000000ae9fef` (**NO** en el CTF `0x4D97…6045`, donde `balanceOf` da siempre 0 — por eso las comprobaciones anteriores salían 0), el manager es `0x30000034706c7d8e12009dab006be20000c031a8`, y Polymarket **autocobra** las ganadoras (~1 h) dejando un `REDEEM` en data-api `/activity` con `usdcSize` = $ cobrados (el bot NO hace redeem). Los combos no cotizan en el CLOB: `midpoint`/`price` dan 404 incluso con el mercado vivo (sólo RFQ). **AUDITORÍA REAL** (77 combos, 26-jul→11-sep; cruze de `/trades` + `/activity` + `balanceOf` en el ERC1155 de combos): era bot (61 combos, stake $5-10) → **25 ganadas cobradas +$195.82** · 3 vendidas (cierres manuales de ago.) +$35.97 · 33 aún en cartera ($285.19, de las que 11 son de las últimas 72 h) ⇒ **PnL realizado +$231.79** (tests antiguos $1-2: +$35.66). El bot, en cambio, reportaba "2 cerradas, 0 ganadas, −$18.37". Ejemplo concreto: combo Chelsea+Liverpool 09-09 20:42 → el bot −$9.74; la wallet cobró $12.19 ⇒ **+$2.45**. Los 5 "sin rastro" de ayer se explican: 4 fueron VENTAS y el 5º (BetBoom 13:33) acabó cobrando +$3.73 al aparecer su REDEEM. **FIX v12.6**: ganador por **OUTCOME** (nombre de la cara) o por **índice 0** si la op antigua no lo guarda (verificado: el orden de `outcomes` del catálogo == el de los tokens CLOB en 7 mercados, y en mercado resuelto el `winner=True` cae sobre el nombre del equipo), cerrado-sin-ganador = ⏳ PENDIENTE, y manda la verdad real (`cobros_wallet()` REDEEM + `saldo_combo_token()` on-chain, con caches 10/5 min y 3 RPC de respaldo); las ops nuevas guardan `outcome` por leg. **FIX v12.7 (petición user: "quiero que corra sola")**: **AUTO-CURACIÓN** — `reauditar_estado()` corre SOLA ~25 s tras cada arranque y cada **4 h** (`AUDITORIA_CADA_H`) dentro de `PASADA_LOCK` y sin depender del modo: reabre las mal archivadas, corrige resultado/PnL (guardando `resultado_antes`/`pnl_antes`), cierra las abiertas ya resueltas, **no toca los cierres manuales 🔒** y sólo avisa por Telegram si cambia algo (🩺 AUTO-CURACIÓN). Guardas: sólo ops de los últimos 45 días, y si el CLOB no devuelve datos de alguna leg (caída/429) la op **no se toca** (`sin_datos`, para no reabrir a ciegas); idempotente (2ª pasada = 0 cambios). `/reauditar [seco]` sigue a mano con saldo on-chain y `/status` muestra la última auditoría. **TESTS**: 11/11 del resolver contra mercados reales + end-to-end de la auditoría sobre estado sintético (corrige la cobrada, reabre la pendiente, respeta la manual, confirma la pérdida real, cierra la abierta resuelta, modo seco no escribe, idempotencia, backup previo) + guard sin-datos. Commits: bot **`4731f473`** · scripts `actualizar_v127.sh` (HASH fijado, backup del estado previo, foto antes/después, publica en diag-public) y `diag_estado_v127.sh` (vuelca `combos_estado.json` a diag-public para contrastarlo op a op). **PUSHEADO el 11-sep** (merge `5b77668` con la rama compartida); **despliegue en Hetzner pendiente**. ⚠️ La cifra "+$231.79" de esta entrada quedó **corregida** más abajo (11 sept ~19:30): el neto verificable de la era del bot es **+$18.52** con **$82.97** aún sin cobrar.
