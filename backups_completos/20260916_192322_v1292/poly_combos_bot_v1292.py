...
# -*- coding: utf-8 -*-
"""
POLY COMBOS BOT v12.9.1 — COMBOS REALES (parlays multi-leg) via RFQ
   v12.9.1 (13-sep-2026): 🩹 un 429 de data-api ya no descarta traders · un fallo
   de red no hace perder fills (ts_ultimo no avanza) · el test del proxy reintenta
   2 min al arrancar · pausa entre wallets 0.6 s.
=====================================================
...
def cmd_start(chat_id):
    texto = (f"🤖 *POLY COMBOS BOT v12.9.1*\n\n"
...
COPY_TOPE_DIA = 12           # señales nuevas/día (tope PROPIO, no el de combos)
...
COPY_MAX_SEÑALES = 900      # tope del histórico en el estado (~60 días a 5/día)
COPY_N_VETERANOS = 3        # v12.9.2: plazas fijas para los más efectivos de la MEMORIA 🧠
COPY_MIN_SEÑALES_VET = 8    # v12.9.2: señales resueltas mínimas para puntuar/vetar
COPY_MIN_ROI_VET = 5.0      # v12.9.2: ROI papel mínimo (%) para plaza de veterano
...
def _lb_get(url, timeout=15, reintentos=3):
    """GET público (sin proxy: leer el ranking no necesita el PC encendido).
    v12.9.1: reintenta con backoff ante 429/5xx/timeout. Un rate-limit pasajero
    NO debe confundirse con "este trader no tiene actividad"."""
    ultimo = None
    for _i in range(max(1, int(reintentos))):
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "poly-combos-bot"})
            with urllib.request.urlopen(req, timeout=timeout) as r:
            ...
            ultimo = e
            if _i + 1 < max(1, int(reintentos)):
                time.sleep(1.0 * (_i + 1))           # 1 s y 2 s
    raise ultimo
...
def perfil_trader(wallet, horas=48, limite=300):
    ...
    try:
        ...
        # v12.9.1: un 429/timeout NO es "trader inactivo". Devolvemos un perfil
        # marcado como error de red para que copy_elegir lo OMITA, no lo descarte.
        log(f"  [copy] ⚠️ error de red leyendo {str(wallet)[:10]}…: {str(e)[:60]}")
        return {"error_red": str(e)[:80], "compras": 0, "ventas": 0, "ratio": 0.0,
                "mercados": 0, "deporte": 0.0, "mediana_usd": 0.0, "ultimo": 0, "n": 0}
...
def filtros_perfil(p):
    ...
    v12.9.1: ok=None significa "no lo sé, ha fallado la red" → ni entra ni se
    descarta: se omite y se reintenta en la siguiente reevaluación."""
    if p and p.get("error_red"):
        return None, f"error de red ({str(p['error_red'])[:40]}): OMITIDO, no descartado"
...
def memoria_traders(estado):
    """🧠 v12.9.2: MEMORIA PERMANENTE por wallet (estado["copy_ranking"]).
    No se poda NUNCA (a diferencia de copy_señales): acumula señales, aciertos,
    pnl y ROI de todo el histórico. Es lo que pidió el user: no olvidar jamás
    qué copytraders son efectivos con nosotros."""
    return estado.setdefault("copy_ranking", {})


def anotar_memoria(estado, s):
    """🧠 Actualiza la memoria al RESOLVERSE una señal s.
    Las anuladas no cuentan (devuelven 0 y no dicen nada del trader)."""
    if s.get("anulada"):
        return None
    w = str(s.get("wallet") or "")
    if not w:
        return None
    mem = memoria_traders(estado)
    g = mem.setdefault(w, {"wallet": w, "nombre": str(s.get("trader") or w)[:24],
                           "senales": 0, "aciertos": 0, "pnl": 0.0,
                           "primera": str(s.get("dia") or ""), "ts_ultima": 0})
    g["nombre"] = str(s.get("trader") or g.get("nombre") or w)[:24]
    g["senales"] = int(g.get("senales") or 0) + 1
    if s.get("senal_acerto"):
        g["aciertos"] = int(g.get("aciertos") or 0) + 1
    g["pnl"] = round(float(g.get("pnl") or 0)
                     + float(s.get("pnl_papel_nuestro") or 0), 3)
    n = max(1, int(g["senales"]))
    g["acierto_pct"] = round(int(g["aciertos"]) / n * 100, 1)
    g["roi"] = round(float(g["pnl"]) / (n * COPY_STAKE_PAPEL) * 100, 1)
    g["ts_ultima"] = int(time.time())
    return g


def backfill_memoria(estado):
    """🧠 Única vez (al desplegar v12.9.2): mete en la memoria las señales YA
    resueltas del histórico, para que la selección 3+2 nazca con veteranos."""
    n = 0
    for s in (estado.get("copy_señales") or []):
        if (s.get("resuelta") and not s.get("anulada")
                and s.get("senal_acerto") is not None):
            anotar_memoria(estado, s)
            n += 1
    if n:
        log(f"  [copy] 🧠 memoria: backfill de {n} señal(es) ya resuelta(s)")
    return n


def copy_elegir(estado, chat_id=None):
    """📡 v12.9.2: selección MIXTA 3+2.
    (1) VETERANOS 🧠 — COPY_N_VETERANOS plazas para los más efectivos de la
        MEMORIA PERMANENTE (≥COPY_MIN_SEÑALES_VET señales resueltas y ROI papel
        ≥COPY_MIN_ROI_VET, ordenados por ROI). Se mantienen aunque SALGAN del
        leaderboard: lo que nos importa es su eficacia CON NOSOTROS, no su puesto.
    (2) NOVATOS — el resto de plazas, del top del leaderboard con los filtros de
        siempre (v12.9.1). La memoria además VETA: el que ya tiene muestra
        suficiente y ROI bajo no vuelve, aunque sea top-1 del ranking.
    Protecciones v12.9.1 intactas: lb-api caído o ≥2 errores de red → se conserva
    la lista anterior (ahora junto a los veteranos) y se reintenta en ~30 min;
    cortocircuito a los 4 errores seguidos."""
    cp = estado_copy(estado)
    mem = memoria_traders(estado)
    elegidos = []
    vistos_w = set()
    vets = sorted((g for g in mem.values()
                   if int(g.get("senales") or 0) >= COPY_MIN_SEÑALES_VET
                   and float(g.get("roi") or 0) >= COPY_MIN_ROI_VET),
                  key=lambda g: (-float(g.get("roi") or 0), -float(g.get("pnl") or 0)))
    for g in vets[:COPY_N_VETERANOS]:
        w = str(g.get("wallet") or "")
        if not w or w in vistos_w:
            continue
        try:
            p = perfil_trader(w)
        except Exception:
        ...
        if not isinstance(p, dict):
            p = {}
        vistos_w.add(w)
        elegidos.append({"nombre": str(g.get("nombre") or w)[:24], "wallet": w,
                         "puesto": 0, "profit": 0.0, "volumen": 0.0,
                         "compras48h": p.get("compras", "?"),
                         "ventas48h": p.get("ventas", "?"),
                         "ratio_ventas": round(float(p.get("ratio") or 0), 2) if p else "?",
                         "mercados": p.get("mercados", "?"),
                         "deporte_pct": round(float(p.get("deporte") or 0) * 100) if p else "?",
                         "mediana_usd": p.get("mediana_usd", "?"),
                         "origen": "veterano",
                         "senales_papel": int(g.get("senales") or 0),
                         "acierto_papel": g.get("acierto_pct"),
                         "roi_papel": g.get("roi"),
                         "elegido_en": int(time.time())})
        log(f"  [copy] 🧠 VETERANO {str(g.get('nombre') or w)[:20]:<20} papel: "
            f"{g.get('senales')} señ · {g.get('acierto_pct')}% acierto · "
            f"ROI {g.get('roi')}% · ${g.get('pnl')}")
        time.sleep(0.6)
    if elegidos:
        log(f"  [copy] 🧠 {len(elegidos)} veterano(s) con plaza fija "
            f"({len(vets)} elegible(s) en la memoria)")
    top = top_traders(COPY_VENTANA, 12, refrescar=True)
    if not top:
        log("  [copy] sin ranking (lb-api no respondió): sigo con la lista anterior")
        ant = cp.get("traders") or []
        if ant:
            return ant
        if elegidos:            # v12.9.2: 1ª vez sin lb-api pero con memoria →
            cp["traders"] = elegidos    # nos quedamos al menos con los veteranos
            cp["ts_eleccion"] = time.time() - COPY_ELEGIR_CADA_S + 1800
            guardar_estado(estado)
        return elegidos
    errores_red = 0                                        # v12.9.1
    for puesto, nom, w, prof, vol in top:
        if not w or str(w) in vistos_w:
            continue
        if len(elegidos) >= COPY_N_TRADERS:
            break
        mg = mem.get(str(w))
        if (mg and int(mg.get("senales") or 0) >= COPY_MIN_SEÑALES_VET
                and float(mg.get("roi") or 0) < COPY_MIN_ROI_VET):
            log(f"  [copy] 🧠 #{puesto} {str(nom)[:20]:<20} VETADO por memoria: "
                f"{mg.get('senales')} señ · ROI {mg.get('roi')}% "
                f"< {COPY_MIN_ROI_VET:.0f}%")
            continue
        p = perfil_trader(w)
        okk, mot = filtros_perfil(p)
        if okk is None:                                    # v12.9.1: red caída
            errores_red += 1
            log(f"  [copy] #{puesto} {str(nom)[:20]:<20} OMITIDO · {mot}")
            if errores_red >= 4:                           # cortocircuito
                log("  [copy] ⚠️ 4 errores de red seguidos: paro el escaneo para no "
                    "bloquear el bucle AUTO (se reintentará en la próxima ronda)")
                break
            time.sleep(2.0)                                # respiro tras un 429
            continue
        ...
        if okk:
            ...
            elegidos.append({"nombre": str(nom)[:24], "wallet": w, "puesto": puesto,
                             ...
                             "origen": "novato",
                             "elegido_en": int(time.time())})
        time.sleep(0.6)                                    # v12.9.1: 0.2 → 0.6 s
        if len(elegidos) >= COPY_N_TRADERS:
            break
    ant = cp.get("traders") or []
    if errores_red >= 2 and ant:
        # v12.9.1 (adaptado al 3+2): con la red fallando la lista nueva sale
        # distorsionada → veteranos 🧠 de la memoria + novatos de la lista anterior.
        nov_ant = [t for t in ant if t.get("origen") != "veterano"
                   and str(t.get("wallet") or "") not in vistos_w]
        huecos = max(0, COPY_N_TRADERS - len(elegidos))
        elegidos = elegidos + nov_ant[:huecos]
        log(f"  [copy] ⚠️ reevaluación con {errores_red} error(es) de red: veteranos 🧠 + "
            f"{min(len(nov_ant), huecos)} novato(s) de la lista anterior "
            "(reintento en ~30 min)")
        cp["traders"] = elegidos
        cp["ts_eleccion"] = time.time() - COPY_ELEGIR_CADA_S + 1800
        guardar_estado(estado)
        return elegidos
    if ant and not elegidos:
        log("  [copy] ⚠️ sin ningún elegido: conservo la lista anterior y reintento en ~30 min")
        cp["ts_eleccion"] = time.time() - COPY_ELEGIR_CADA_S + 1800
        guardar_estado(estado)
        return ant
    cp["traders"] = elegidos
    ...
    guardar_estado(estado)
    nv = sum(1 for t in elegidos if t.get("origen") == "veterano")
    log(f"  [copy] vigilando {len(elegidos)} traders ({nv} 🧠 veteranos): "
        + ", ".join(t["nombre"] for t in elegidos))
    if chat_id and elegidos:
        lines = ""
        for i, t in enumerate(elegidos, 1):
            if t.get("origen") == "veterano":
                lines += (f"   {i}. *{t['nombre']}* 🧠 veterano · papel: "
                          f"{t.get('senales_papel', '?')} señ · "
                          f"{t.get('acierto_papel', '?')}% acierto · "
                          f"ROI {t.get('roi_papel', '?')}%\n")
            else:
                lines += (f"   {i}. *{t['nombre']}* {_din_lb(t['profit'])} · "
                          f"{t['compras48h']} compras/48h · {t['deporte_pct']}% deporte\n")
        enviar(chat_id, f"📡 *Copy-trading en papel*: ya vigilo a {len(elegidos)} traders\n"
                        f"{lines}_Selección v12.9.2: veteranos 🧠 por eficacia demostrada "
                        f"en nuestro papel + novatos del top {COPY_VENTANA} filtrados "
                        "(fuera market makers, inactivos y concentrados). "
                        "Sus compras son SEÑALES: no compro nada._")
    return elegidos


def fills_recientes(wallet, desde_ts, limite=200):
    """Compras de una wallet posteriores a desde_ts (más recientes primero).
    v12.9.1: devuelve None si ha fallado la RED (antes devolvía [], que se
    confundía con "no ha comprado nada" y hacía perder esos fills para siempre)."""
    try:
        act = _lb_get(f"{DATA_API}/activity?user={wallet}&limit={limite}&type=TRADE") or []
    except Exception as e:
        log(f"  [copy] ⚠️ fills de {str(wallet)[:10]}…: {str(e)[:60]}")
        return None
...
def resolver_señales(estado):
    ...
    for s in pend[:COPY_RESOLVER_POR_RONDA]:
        ...
        s["resuelta"] = True
        anotar_memoria(estado, s)      # 🧠 v12.9.2: memoria permanente por wallet
        n += 1
    return n
...
def texto_copy(estado):
    ...
    if r["traders"]:
        ...
        for x in r["traders"][:COPY_N_TRADERS]:
            if x.get("origen") == "veterano":
                t += (f"   · *{x['nombre']}* 🧠 veterano · papel: {x.get('senales_papel', '?')} señ · "
                      f"{x.get('acierto_papel', '?')}% acierto · ROI {x.get('roi_papel', '?')}%\n")
            else:
    ...
    mem = memoria_traders(estado)
    if mem:
        t += "\n🧠 *Memoria permanente* (palmarés acumulado · nunca se olvida):\n"
        for g in sorted(mem.values(), key=lambda x: -float(x.get("roi") or 0))[:8]:
            t += (f"   · {str(g.get('nombre') or '?')[:20]:<20} {int(g.get('senales') or 0):>3} señ · "
                  f"{float(g.get('acierto_pct') or 0):>5.1f}% · ${float(g.get('pnl') or 0):>+8.2f} · "
                  f"ROI {float(g.get('roi') or 0):>+6.1f}%\n")
    t += ("\n🎯 *Para pasar a Fase 2* (convertir la señal en un combo propio por RFQ): "
          "≥100 señales resueltas (o ≥40 EN RANGO combo) y ROI positivo AL PRECIO "
          "NUESTRO, y sólo con veteranos 🧠 de ROI ≥+10% y ≥20 señales.\n"
...
def copy_pasada(chat_id):
    ...
    estado = cargar_estado()
    cp = estado_copy(estado)
    if not estado.get("copy_ranking_v1"):
        # 🧠 v12.9.2: backfill ÚNICO de la memoria con el histórico ya resuelto
        # + ts_eleccion=0 para que la selección 3+2 entre en vigor EN ESTA RONDA.
        backfill_memoria(estado)
        cp["ts_eleccion"] = 0.0
        estado["copy_ranking_v1"] = True
        guardar_estado(estado)
    ...
    # v12.9.1: suelo de 6 h — tras una caída larga no pedimos una ventana que
    # limit=200 no puede cubrir.
    desde = max(float(cp.get("ts_ultimo") or (time.time() - 3600)), time.time() - 6 * 3600)
    nuevas = 0
    fusiones = 0
    tope = False
    fallos_red = 0
    for t in traders:
        if tope:
            break
        _fills = fills_recientes(t.get("wallet"), desde)
        if _fills is None:                                 # v12.9.1: error de red
            fallos_red += 1
            continue
        for f in _fills:
    ...
    cp["vistos"] = limpios
    # v12.9.1: con errores de red NO avanzamos el reloj → la próxima ronda vuelve a
    # mirar la misma ventana (el dedup de vistos impide señales duplicadas).
    if fallos_red:
        log(f"  [copy] ⚠️ {fallos_red} trader(s) con fallo de red: ts_ultimo sin avanzar")
    else:
...
def main():
    ...
    log(f"v12.9.2 cargado · modo={MODO_OPERACION} · stake={stake_txt()} · max {MAX_OPS_DIA}/día · prob ≥{int(PROB_MIN_AUTO * 100)}%")
    log(f"Proxy: {PROXY_URL}")
    # v12.9.1: tras un reinicio de la MÁQUINA, Tailscale tarda en levantar. Antes el
    # bot arrancaba con "Test proxy: FALLO" y se quedaba así; ahora reintenta 2 min.
    status = None
    for _i in range(12):
        status, body = http_get("https://api.telegram.org", timeout=10)
        if status:
            break
        log(f"  Test proxy: fallo (intento {_i + 1}/12) — reintento en 10 s")
        time.sleep(10)
    log(f"Test proxy: {status if status else 'FALLO (tras 12 intentos en 2 min)'}")
